...
void loop() {
  ...
  */
  ...
  if (temperaturaoleo > maxTEMPoleo || temperaturacab > maxTEMPcab || RPM >= maxRPM || (RPM < minRPM && RPM != 0))
  {
    digitalWrite(ledR, HIGH);
    digitalWrite(ledG, LOW);
    digitalWrite(ledY, LOW);
  }
  else if (temperaturaoleo > dangerTEMPoleo || temperaturacab > dangerTEMPcab || RPM >= medRPM)
  {
